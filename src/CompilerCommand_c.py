...
def get_preprocessing_options(options):
    ...
    for option in options:
        if option not in\
               CompilerCommand_c.compilation_stop_level_by_option.keys()\
               and option != "-o":
...
class CompilerCommand_c(CompilerCommand.CompilerCommand):
    ...
    def __init__(self, command_arguments):
        ...
        #Contain, the path of the compiler (the true path)
        self.__local_compiler_path = None
    ...
    def get_stop_step(self):
        ...
        for arg in self.__command_arguments[1:]:
            if arg in \
                   CompilerCommand_c.compilation_stop_level_by_option.keys() \
                   and CompilerCommand_c.compilation_stop_level_by_option[arg]\
                   < self.__stop_step:
                self.__stop_step = CompilerCommand_c.\
    ...
    def get_output_files(self):
        ...
        if "-o" in self.__command_arguments:
            ...
            for (index, file) in self.get_input_files():
                ...
                if file_output_name != "":
                    res.append(pathfile_output_name)
            ...
            if get_start_step_of_file(file[1]) == 1:
                ...
                if __debug__:
                    syslog.syslog(syslog.LOG_DEBUG | syslog.LOG_DAEMON, 
                                  ...
                                  + " " + file[1])
                preprocess_out = os.popen(self.get_local_compiler_path() + \
                                          " -E " + " ".join(\
    ...
    def get_local_compiler_path(self):
        if self.__local_compiler_path == None:
            ...
            keys_match = [
                ("C_PATH", "compiler_path", "C_PATH")
                ]

            command_line_match=[]
            
            default_values = {
                "C_PATH" : commands.getoutput("which cc")
                }
    ...
    def check_values(self, config_values):
        #DIXME Check that the compiler exists
        return ((), 1)
